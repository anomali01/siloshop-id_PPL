  ...
  try {
    ...
    if (couponCode && coupon.forNewUser) {
      ...
      if (userorders.length > 0) {
        return NextResponse.json(
          { error: "Kupon valid untuk pengguna baru" },
          { status: 400 }
        );
      }
    }
    ...
    if (couponCode && coupon.forMember) {
      if (!isPlusMember) {
        return NextResponse.json(
          { error: "Kupon valid untuk members only" },
          { status: 400 }
        );
      }
    }
    ...
    for (const [storeId, sellerItems] of ordersByStore.entries()) {
      ...
      fullAmout += parseFloat(total.toFixed(2));
    ...
    return NextResponse.json({ message: "Orders Place Successfully" });
  } catch (error) {
    console.error(error);
    return NextResponse.json(
