import { Code, PlusIcon, SquarePenIcon, XIcon } from "lucide-react";
...
const OrderSummary = ({ totalPrice, items }) => {
  ...
  const handleCouponCode = async (event) => {
    ...
    try {
      if (!user) {
        return toast("Mohon login untuk memproses");
      }
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      if (!user) {
        return toast("Mohon login untuk melakukan pemesanan");
      }
      if (!selectedAddress) {
        return toast("Mohon pilih alamat");
      }
      const token = await getToken();
  ...
  return (
    <div className="w-full max-w-lg lg:max-w-[340px] bg-slate-50/30 border border-slate-200 text-slate-500 text-sm rounded-xl p-7">
      ...
      <div className="pb-4 border-b border-slate-200">
        <div className="flex justify-between">
          ...
          <div className="flex flex-col gap-1 font-medium text-right">
            ...
            <p>
              <Protect plan={"plus"} fallback={`${currency}2000`}>
                Gratis
              </Protect>
            </p>
            {coupon && (
        ...
        {!coupon ? (
          ...
          >
            ...
            />
            <button className="bg-red-500 text-white px-3 rounded hover:bg-red-700 active:scale-95 transition-all">
              Apply
            </button>
          </form>
        ) : (
      ...
      <button
        ...
        className="w-full bg-red-500 text-white py-2.5 rounded hover:bg-red-700 active:scale-95 transition-all"
      >
        Check Out
      </button>
